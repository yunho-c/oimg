...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    const float LEVEL_TILT_INFLUENCE = 0.175;
    ...
    for (int i = 0; i < NUM_LAYERS; i++) {
        ...
        float phaseInterval  = fi * 0.4;       // Offsets the rhythm/timing
        ...
        float levelTilt = pos.x * fi * LEVEL_TILT_INFLUENCE;
        ...
        float waveShape = 0.30 * sin(x * 1.2 + t + phaseInterval)
                        ;
